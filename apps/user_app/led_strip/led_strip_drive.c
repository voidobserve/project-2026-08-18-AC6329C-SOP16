...
{
    gpio_set_die(LED_DRIVER_R_PIN, 1);
    gpio_direction_output(LED_DRIVER_R_PIN, 0);

    gpio_set_die(LED_DRIVER_G_PIN, 1);
    gpio_direction_output(LED_DRIVER_G_PIN, 0);

    gpio_set_die(LED_DRIVER_B_PIN, 1);
    gpio_direction_output(LED_DRIVER_B_PIN, 0);

    gpio_set_die(LED_DRIVER_W_PIN, 1);
    gpio_direction_output(LED_DRIVER_W_PIN, 0);
}

/**************************  风扇IO口初始化   ***********/
void fan_gpio_init(void)
{
    gpio_set_pull_down(FAN_DRIVER_PIN, 0);
    gpio_set_pull_up(FAN_DRIVER_PIN, 0);
    gpio_direction_output(FAN_DRIVER_PIN, 1);
}
...
void close_fan(void)
{
    gpio_direction_output(FAN_DRIVER_PIN, 0);
}
...
void led_pwm_init(void)
{
    ...
    pwm_p_data.h_pin = LED_DRIVER_R_PIN;            // 任意引脚
    pwm_p_data.l_pin = -1;                          // 任意引脚,不需要就填-1
    //两个引脚的波形, 0: 同步,  1: 互补，互补波形的占空比体现在H引脚上
    ...
    pwm_p_data.frequency = 1000;                    // 1KHz
    pwm_p_data.duty = 0;                            // 上电输出0%占空比
    pwm_p_data.h_pin = LED_DRIVER_G_PIN;            // 任意引脚
    pwm_p_data.l_pin = -1;                          // 任意引脚,不需要就填-1
    //两个引脚的波形, 0: 同步,  1: 互补，互补波形的占空比体现在H引脚上
    pwm_p_data.complementary_en = 0;
    mcpwm_init(&pwm_p_data);
    ...
    pwm_p_data.frequency = 1000;                    // 1KHz
    pwm_p_data.duty = 0;                            // 上电输出0%占空比
    pwm_p_data.h_pin = LED_DRIVER_B_PIN;            // 任意引脚
    pwm_p_data.l_pin = -1;                          // 任意引脚,不需要就填-1
    //两个引脚的波形, 0: 同步,  1: 互补，互补波形的占空比体现在H引脚上
    pwm_p_data.complementary_en = 0;
    mcpwm_init(&pwm_p_data);
    ...
    pwm_p_data.frequency = 1000;                    // 1KHz
    pwm_p_data.duty = 0;                            // 上电输出0%占空比
    pwm_p_data.h_pin = LED_DRIVER_W_PIN;            // 任意引脚
    pwm_p_data.l_pin = -1;                          // 任意引脚,不需要就填-1
    //两个引脚的波形, 0: 同步,  1: 互补，互补波形的占空比体现在H引脚上
    pwm_p_data.complementary_en = 0;
    mcpwm_init(&pwm_p_data);
