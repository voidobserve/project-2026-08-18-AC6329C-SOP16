...
void fc_data_init(void)
{
    ...
    fc_effect.b = 255;
    fc_effect.report_app_speed = 80;
    // 这里根据要反馈给app的值来设置速度值
    fc_effect.dream_scene.speed =
        dynamic_speed_convert_by_percent(fc_effect.report_app_speed);
    fc_effect.music.m = 1;
...
void fb_led_bright_state(void)
{
    ...
    Send_buffer[0] = 0x04;
    Send_buffer[1] = 0x03;
    Send_buffer[2] = (uint16_t)fc_effect.b * 100 / 255; //
    extern void zd_fb_2_app(u8 * p, u8 len);
    zd_fb_2_app(Send_buffer, 3);
}
...
void fb_led_speed_state(void)
{
    ...
    Send_buffer[1] = 0x04;
    // 将速度值映射到 0 ~ 100
    Send_buffer[2] = fc_effect.report_app_speed;
    extern void zd_fb_2_app(u8 * p, u8 len);
    zd_fb_2_app(Send_buffer, 3);
}
...
void special_w_close(void)
{
    ...
    if (fc_effect.on_off_flag == DEVICE_OFF && fc_effect.w != 0) {
        ...
        if (temp_w_bright > 0) {
            ...
            temp_w_bright--;
            mcpwm_set_duty(pwm_ch3,
                           fc_effect.w * 10000 / 255 * temp_w_bright / 255);
        }
                                ...
                                unsigned short *led_place)
{
    ...
    if (fc_effect.smear_adjust.smear_tool == IS_drum) //油桶
    {
        m_hsv_to_rgb(&fc_effect.smear_adjust.rgb[0].r,
                     ...
                     &fc_effect.smear_adjust.rgb[0].b, colour->h_val,
                     colour->s_val, colour->v_val);
               ...
               (fc_effect.smear_adjust.smear_tool == IS_eraser)) //橡皮擦
    {
        m_hsv_to_rgb(&fc_effect.smear_adjust.rgb[*led_place].r,
                     ...
                     &fc_effect.smear_adjust.rgb[*led_place].b, colour->h_val,
                     colour->s_val, colour->v_val);
    }
}
/*----------------------------------涂抹模式----------------------------------*/
...
static uint16_t ls_smear_adjust_effect(void)
{
    ...
    for (num = 0; num < max; ++num) {
        WS2812FX_setPixelColor_rgb(num, fc_effect.smear_adjust.rgb[num].r,
...
static void fc_smear_adjust(void)
{
    //WS2812FX_stop();
    WS2812FX_setSegment_colorOptions(0, 0, fc_effect.led_num - 1,
                                     &ls_smear_adjust_effect, BLUE, 100, 0);
    WS2812FX_start();
}
...
void three_seg_meteor(void)
{
    ...
    WS2812FX_setSegment_colorOptions(1,     //第0段
                                     0, 59, //起始位置，结束位置
    ...
    WS2812FX_setSegment_colorOptions(2,     //第0段
                                     0, 59, //起始位置，结束位置
                                     &WS2812FX_mode_color_sweep_random, //效果
                                     WHITE, //颜色，WS2812FX_setColors设置
                                     800,   //速度
...
/**
 * @brief 传入 0 ~ 100 的百分比值，映射到 500 ~ 10 动态模式下的速度值
 *      根据百分比，转换成对应的速度值
 * 
 */
u16 dynamic_speed_convert_by_percent(u8 percent)
{
    return (500 - 10) - (uint16_t)percent * (500 - 10) / 100;
}

//
/**
 * @brief 传入 10 ~ 500 动态模式下的速度值，映射到 100% ~ 0% 对应的百分比值
 *      根据速度值，转换成对应的百分比
 * 
 */
u8 dynamic_speed_convert_by_speed_val(u16 speed_val)
{
    return 100 - ((uint16_t)speed_val - 10) * 100 / (500 - 10);
}

// 最慢500ms一帧,s:1-100%
void ls_set_speed(uint8_t speed)
{
    fc_effect.report_app_speed = speed;

    // 将传参映射到 10 ~ 500，传参的值越大，映射值越小
    fc_effect.dream_scene.speed = dynamic_speed_convert_by_percent(speed);
...
void speed_fast(void)
{
    ...
    // 速度值变化之后，需要更新对应的百分比值
    fc_effect.report_app_speed =
        dynamic_speed_convert_by_speed_val(fc_effect.dream_scene.speed);
...
void set_fc_effect(void)
{
    ...
    if (fc_effect.on_off_flag == DEVICE_ON) {
        ...
        case IS_light_music:
            fc_effect.w = 0;
            // 声控动画使用最大亮度，但是不改变 fc_effect.b 的值
