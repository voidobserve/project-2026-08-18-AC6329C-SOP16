...
unsigned short dp_extract_data_handle(unsigned char *buff)
{
    ...
    case DPID_DRAW_TOOL: //涂抹功能(可下发可上报)
        ...
        effect_smear_adjust_updata((smear_tool_e)dp_draw_tool.tool,
                                   &(dp_draw_tool.colour),
                                   &dp_draw_tool.led_place);
...
void parse_zd_data(unsigned char *LedCommand)
{
    ...
    if (LedCommand[0] == 0x01 && LedCommand[1] == 0x03) //与APP同步数据
    {
        ...
        printf("\n rgbw");
        int result = ble_comm_att_send_data(
            fd_handle, ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE, Send_buffer, 9,
        ...
        //     Send_buffer[8] = 0;
        ble_comm_att_send_data(fd_handle,
                               ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE,
                               Send_buffer, 9, ATT_OP_AUTO_READ_CCC);
        ...
        Send_buffer[6] = 0x04;
        Send_buffer[7] = 0x04;
        Send_buffer[8] = fc_effect.report_app_speed;
        // app_send_user_data(ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE, Send_buffer,9, ATT_OP_AUTO_READ_CCC);
        ble_comm_att_send_data(fd_handle,
                               ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE,
                               Send_buffer, 9, ATT_OP_AUTO_READ_CCC);
...

    {
        ...
        if (fc_effect.on_off_flag == DEVICE_ON) {
            ...
            if (LedCommand[0] == 0x06 && LedCommand[1] == 0x04) {
                extern void set_static_mode(u8 r, u8 g, u8 b);
                // 声控模式下使用最大亮度，但是不改变 fc_effect.b 的值
            ...
            if (LedCommand[0] == 0x2F && LedCommand[1] == 0x08) { 
                if (fc_effect.base_ins.motor_on_off == 0) {
                    // 开电机 
                    ...
                    ble_comm_att_send_data(
                        fd_handle, ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE,
                        Send_buffer, 9, ATT_OP_AUTO_READ_CCC); 
