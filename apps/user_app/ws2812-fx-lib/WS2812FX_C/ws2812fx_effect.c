...
{
    /*
        将速度值限制到 300 及以下，相当于去掉了最快的3个挡位
        
        out = 300 + (in - 10) * (500 - 300) / (500 - 10)
        out = 300 + (in - 10) * 200 / 490
    */
    volatile uint16_t speed;
    // 将速度值映射到 300 ~ 500
    speed = 300 + ((uint16_t)fc_effect.dream_scene.speed - 10) * (500 - 300) /
                      (500 - 10);
...
uint16_t music_2_side_oc(void)
{
    ...
    if (m_oc.act == E_RISE) {
        ...
        if (m_oc.c_pos1 > _seg_len - m_oc.trg &&
            m_oc.c_pos1 > 0) //倒序上升位置，
        {
            m_oc.c_pos1--;
        }
    }
